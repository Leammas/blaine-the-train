...
actual recent sessions in COROS history. Trait (backtest 2026-08-22):
tolerates isolated long endurance days well when fresh — plan big days
confidently, but only planned ones, absorbed by a following down week.
...
- ≤ 9 training hours/week INCLUDING strength. Exception: optional easy
  mobility runs (≤8 km) on strength days sit outside the core plan and may
  push a week over the cap; the cap binds the core (non-optional) plan.
- Weekday morning sessions ≤ 1 h (home by 07:00). Doubles allowed.
- Thursday social ride is fixed (see reference/routes.md); counts as bike
  quality. SEASONAL: fixed only until ~mid-October 2026 (cold/dark pause);
  from macro Phase 3 Thursday is a normal run/strength slot until the athlete
  says the ride is back on.
...
- Goal: maintain the target weight in `athlete.private.md`. No food logging.
  Daily kcal targets per coach-rules.md formula; carb emphasis on/before long
  or hard days.
